...
public class FAQInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		faqboardVo.setFaqCategory(faqCategory);
		faqboardVo.setFaqTitle(faqTitle);
		faqboardVo.setFaqContent(faqContent);
		
		int inResult = faqboardDao.insertFaqboard(faqboardVo);
		
		/*
		 * String driver="oracle.jdbc.driver.OracleDriver"; String
		 * url="jdbc:oracle:thin:@localhost:1521:xe"; String id = "passion"; String pw =
		 * "passion"; Connection conn = null; Statement stmt = null;
		 * 
		 * try { Class.forName(driver); conn=DriverManager.getConnection(url,id,pw);
		 * stmt=conn.createStatement(); String sql = "INSERT INTO FAQ_BOARD"+
		 * "(FAQ_NO,MEMBER_NO,FAQ_CATEGORY,FAQ_TITLE,FAQ_CONTENT,FAQ_DATE)"+
		 * "VALUES(FAQ_SEQ.NEXTVAL, 0 ,'"+faqCategory+"','"+faqTitle+"', '"
		 * +faqContent+"', SYSDATE)";
		 * 
